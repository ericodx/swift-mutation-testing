...
struct ProcessLauncher: Sendable, ProcessLaunching {
    ...
    func launchCapturingDeferred(
        ...
        let result = try await withTaskCancellationHandler {
            try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<CapturedOutput, any Error>) in
                self.startDeferredCapturingProcess(
                    process, killedByUs: killedByUs, timeout: timeout,
                    sandboxPath: sandboxPath,
                    capture: CaptureTarget(fileHandle: fileHandle, tempURL: tempURL),
                    continuation: continuation
    ...
    private func startDeferredCapturingProcess(
        _ process: Process,
        killedByUs: KilledByUsFlag,
        timeout: Double,
        sandboxPath: String,
        capture: CaptureTarget,
        continuation: CheckedContinuation<CapturedOutput, any Error>
    ) {
        let timeoutTask = Task {
            try await Task.sleep(for: .seconds(timeout))
            killedByUs.mark()
        ...
        process.terminationHandler = { terminated in
            ...
            continuation.resume(returning: CapturedOutput(exitCode: exitCode, output: output, cleanup: cleanup))
        }
